...
{
    public class AddCategoryFromExcel : IAddCategoryFromExcel
    {
        private IConfiguration configuration;
        private IWebHostEnvironment webHostEnvironment;
        public AddCategoryFromExcel(IConfiguration configuration, IWebHostEnvironment webHostEnvironment)
        {
            this.configuration = configuration;
            this.webHostEnvironment = webHostEnvironment;
        }
        public DataTable CategoryDataTable(string path)
        {
        ...
        public void ImportCategory(DataTable category)
        {
            ...
            using (SqlConnection scon = new SqlConnection(sqlconn))
            {
                ...
                {
                    sqlBulkCopy.DestinationTableName = "Categories";
                    foreach (DataColumn column in category.Columns)
                    {
                        // Ánh xạ cột
                        sqlBulkCopy.ColumnMappings.Add(column.ColumnName, column.ColumnName);
                    }
                    sqlBulkCopy.WriteToServer(category);
                }
